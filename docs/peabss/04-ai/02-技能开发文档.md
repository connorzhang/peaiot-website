...
#### MCP服务
- **接口**：`POST /mcp`
- **协议**：JSON-RPC 2.0，Trae Solo HTTP MCP 统一入口
- **列表方法**：`skills/list`
- **调用方法**：`skills/call`
- **参数**：`{"jsonrpc":"2.0","method":"skills/call","params":{"name":"skill_name","arguments":{"param1":"value1"}},"id":1}`
- **返回**：MCP `content` 结构，执行文本为 JSON 字符串
...
- **列表方法**：`tools/list`
- **调用方法**：`tools/call`
- **参数**：`{"jsonrpc":"2.0","method":"tools/call","params":{"name":"tool_name","arguments":{"param1":"value1"}},"id":1}`
- **返回**：MCP `content` 结构，执行文本为 JSON 字符串

### 4. Trae Solo 接入方式

- **配置文件**：`c:\Users\connor\AppData\Roaming\Trae\User\mcp.json`
- **示例文件**：`.trae/mcp.example.json`
- **URL**：`http://127.0.0.1:1875/mcp`
- **鉴权**：`Authorization: Bearer <api_key_or_portal_jwt>`
- **智能体工具**：`agents_list`、`agent_execute`
...
### 1. 前端集成
- 通过 `POST /mcp` 标准 JSON-RPC 接口调用 MCP 工具和技能
- 展示技能执行结果
- 提供技能参数配置界面
